...
from subprocess import Popen, PIPE, TimeoutExpired, DETACHED_PROCESS, CREATE_NO_WINDOW
#import subprocess
...
def convert_from_path(
    ...
    for _ in range(thread_count):
        ...
        #startup_info.dwFlags |= subprocess.CREATE_NO_WINDOW
        #print('this is attempting to open a new console for pdftoppm')
        processes.append(
            (thread_output_file, Popen(args, env=env, stdout=PIPE, stderr=PIPE, creationflags=CREATE_NO_WINDOW))
        )
...
def _get_poppler_version(command, poppler_path=None, timeout=None):
    ...
    if poppler_path is not None:
        env["LD_LIBRARY_PATH"] = poppler_path + ":" + env.get("LD_LIBRARY_PATH", "")
    proc = Popen(command, env=env, stdout=PIPE, stderr=PIPE, creationflags=CREATE_NO_WINDOW)
